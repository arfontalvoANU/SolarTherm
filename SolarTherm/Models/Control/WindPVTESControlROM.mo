...
model WindPVTESControlROM
  ...
  parameter SI.MassFlowRate m_flow_boiler_des = 100.0 "Reference HTF mass flow rate to the boiler at the target enthalpy";
  parameter SI.HeatFlowRate Q_flow_boiler_des = 200e6 "Boiler design input heat rate";
  ...
  SI.MassFlowRate m_flow_guess(start=0.0) "Required mass flow rate of the heater assuming it is on and not curtailed.";
  SI.MassFlowRate m_flow_boiler_dem = m_flow_boiler_des*Q_flow_demand/Q_flow_boiler_des "Instantaneous boiler demand mass flow rate at design enthalpy (kg/s)";
...
initial algorithm
  ...
  if m_flow_guess > m_flow_tol then
    if m_flow_guess < m_flow_boiler_dem + m_flow_tol then //no overshoot
      Flow_State := 1;
    else //overshot
      Flow_State := 2;
    end if;
...
algorithm
  ...
  when m_flow_boiler_signal < 0.1 * m_flow_boiler_des then //take this as shutdown
  ...
  when m_flow_guess > m_flow_tol then
    if m_flow_guess < m_flow_boiler_dem + m_flow_tol then //no overshoot
      Flow_State := 1;
    else //overshot
      Flow_State := 2;
  ...
  when m_flow_guess < m_flow_min then
    Flow_State := 0;
  end when;
  when m_flow_guess > m_flow_boiler_dem + m_flow_tol then
    Flow_State := 2;
  end when;
  when m_flow_guess < m_flow_boiler_dem then
    if m_flow_guess > m_flow_tol then //no overshoot
      Flow_State := 1;
    else
      Flow_State := 0;
    end if;
...
equation
  if Q_flow_heater_raw >= Q_flow_demand then
    m_flow_guess = (Q_flow_heater_raw + m_flow_boiler_dem*(h_boiler_outlet-h_tank_bot))/(h_target-h_tank_bot);
  else
    m_flow_guess = Q_flow_heater_raw/(h_target-h_boiler_outlet);
  end if;
  ...
  elseif Control_State == 3 then //Heater is curtailed exactly to the process heat flow rate.
    Q_TES_in = 0.0;
    m_flow_heater_signal = max(m_flow_0,m_flow_boiler_dem);
