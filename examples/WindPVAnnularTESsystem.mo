...
model WindPVAnnularTESsystem
  ...
  parameter Modelica.SIunits.Power PV_ref_size = 50.0e6;
  parameter Modelica.SIunits.Power Wind_ref_size = 50.0e6;
  //Results
  ...
  parameter Modelica.SIunits.Length L_pipe = 50.0;
  parameter Modelica.SIunits.Length D_pipe = 0.050;
  parameter Modelica.SIunits.Length D_solid = 0.075; 
  ...
  parameter Modelica.SIunits.Temperature T_max = 500.0 + 273.15 "Maximum system temperature (K)";
  parameter Modelica.SIunits.Temperature T_boiler_start = 400.0 + 273.15 "Temperature above-which TES can start discharge (K)"; //50K buffer
  parameter Modelica.SIunits.Temperature T_boiler_min = 350.0 + 273.15 "Temperature below-which TES stops discharge (K)";
  parameter Modelica.SIunits.Temperature T_heater_max = 300.0 + 273.15 "Temperature above-which TES stops charging (K)";
  parameter Modelica.SIunits.Temperature T_heater_start = 250.0 + 273.15 "Temperature below-which TES can start charging (K)"; //50k buffer
  parameter Modelica.SIunits.Temperature T_min = 150.0 + 273.15 "Minimum system temperature (K)";
    //Legacy terms from the CSP field. PB (power block) refers to the boiler, Recv (receiver) refers to the electric heater.

  //Level-Controls
  parameter Modelica.SIunits.Time t_stor_start_dis = 1.0*3600.0 "Number of effective storage seconds stored before TES can start discharging (1 hour)";  
  ...
  SolarTherm.Models.Storage.Thermocline.Annular.SM_SingleTank thermocline_Tank(
  ...
  SolarTherm.Models.Fluid.Valves.PBS_TeeJunction Splitter_Bot(redeclare package Medium = Medium) 
    annotation(Placement(visible = true, transformation(origin = {31, -43.8998}, extent = {{17, 0}, {-17, -22.039}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.RealExpression Tamb(y = 298.15) 
    annotation(Placement(visible = true, transformation(origin = {1, 0}, extent = {{-9, -12}, {9, 12}}, rotation = 0)));

  Modelica.Blocks.Sources.RealExpression p_amb(y = 101325) 
    annotation(Placement(visible = true, transformation(origin = {107, 2.22045e-16}, extent = {{11, -12}, {-11, 12}}, rotation = 0)));
  ...
  SolarTherm.Models.Control.WindPV_Thermocline_Control Control(
    ...
    t_wait = 1.0 * 3600.0)
      annotation(Placement(visible = true,transformation(origin = {114, 26},extent = {{-10, -10}, {10, 10}},rotation = 0)));
  ...
  Modelica.Blocks.Sources.CombiTimeTable scheduler(
    ...
    tableName = "Q_flow", 
    tableOnFile = true) 
      annotation(Placement(visible = true,transformation(origin = {184, 38},extent = {{10, -10}, {-10, 10}},rotation = 0)));
  ...
  SolarTherm.Models.CSP.CRS.Receivers.Basic_Heater basic_Heater(
    redeclare package Medium = Medium,
    ...
    T_cold_set = T_cold_set, 
    T_hot_set = T_hot_set) 
      annotation(Placement(visible = true,transformation(origin = {-46, 10},extent = {{-10, -10}, {10, 10}},rotation = 0)));
  ...
  Modelica.Blocks.Math.Add Grid_Sum(
    ...
    k2 =  P_renewable_des *(1.0 - PV_fraction) / Wind_ref_size) 
      annotation(Placement(visible = true,transformation(origin = {-84, 18},extent = {{-10, -10}, {10, 10}},rotation = 0)));
...
//grid_input.Q_defocus_y = min(gridInput.grid_input.y[1], scheduler.y[1] * (h_salt_hot_set - h_salt_cold_set));
  ...
  connect(Control.Q_flow_curtail, basic_Heater.Q_flow_curtail) annotation(
    Line(points = {{103, 17}, {-10, 17}, {-10, -12}, {-72, -12}, {-72, 2}, {-57, 2}, {-57, 5}}, color = {0, 0, 127}));
  connect(scheduler.y[1], Control.Q_flow_demand) annotation(
  ...
  connect(basic_Heater.Q_flow_heater_raw, Control.Q_flow_heater_raw) annotation(
