...
model SectionROM "Heat transfer model of thermocline tank with spherical fillers"
  ...
  Fluid_Package.State fluid[Nz] "Fluid object array";
  //(each h_start = hf_min)
...
//Fluid equations
  if State == 1 then
    der_hf[1] := ((-2 * kf[1] * kf[2] / (kf[1] + kf[2]) * (Tf[1] - Tf[2]) / dz ^ 2) + rhof_avg * uf_avg / epsilon * (hf[1] - hf[2]) / dz - hv[1] * (Tf[1] - Ts[1]) / epsilon - U_bot * (Tf[1] - T_amb) / (epsilon * dz) - U_wall * CN.pi * D_tank * (Tf[1] - T_amb) / (epsilon * A)) / rhof_avg;
    h_out := hf[1];
    for i in 2:Nz - 1 loop
      der_hf[i] := ((-2 * kf[i] * kf[i - 1] / (kf[i] + kf[i - 1]) * (Tf[i] - Tf[i - 1]) / dz ^ 2) - 2 * kf[i] * kf[i + 1] / (kf[i] + kf[i + 1]) * (Tf[i] - Tf[i + 1]) / dz ^ 2 + rhof_avg * uf_avg / epsilon * (hf[i] - hf[i + 1]) / dz - hv[i] * (Tf[i] - Ts[i]) / epsilon - U_wall * CN.pi * D_tank * (Tf[i] - T_amb) / (epsilon * A)) / rhof_avg;
    end for;
    der_hf[Nz] := ((-2 * kf[Nz - 1] * kf[Nz] / (kf[Nz - 1] + kf[Nz]) * (Tf[Nz] - Tf[Nz - 1]) / dz ^ 2) + rhof_avg * uf_avg / epsilon * (hf[Nz] - h_in) / dz - hv[Nz] * (Tf[Nz] - Ts[Nz]) / epsilon - U_wall * CN.pi * D_tank * (Tf[Nz] - T_amb) / (epsilon * A) - U_top * (Tf[Nz] - T_amb) / (epsilon * dz)) / rhof_avg;
  else
    der_hf[1] := ((-2 * kf[1] * kf[2] / (kf[1] + kf[2]) * (Tf[1] - Tf[2]) / dz ^ 2) + rhof_avg * uf_avg / epsilon * (h_in - hf[1]) / dz - hv[1] * (Tf[1] - Ts[1]) / epsilon - U_bot * (Tf[1] - T_amb) / (epsilon * dz) - U_wall * CN.pi * D_tank * (Tf[1] - T_amb) / (epsilon * A)) / rhof_avg;
    for i in 2:Nz - 1 loop
      der_hf[i] := ((-2 * kf[i] * kf[i - 1] / (kf[i] + kf[i - 1]) * (Tf[i] - Tf[i - 1]) / dz ^ 2) - 2 * kf[i] * kf[i + 1] / (kf[i] + kf[i + 1]) * (Tf[i] - Tf[i + 1]) / dz ^ 2 + rhof_avg * uf_avg / epsilon * (hf[i - 1] - hf[i]) / dz - hv[i] * (Tf[i] - Ts[i]) / epsilon - U_wall * CN.pi * D_tank * (Tf[i] - T_amb) / (epsilon * A)) / rhof_avg;
    end for;
    der_hf[Nz] := ((-2 * kf[Nz - 1] * kf[Nz] / (kf[Nz - 1] + kf[Nz]) * (Tf[Nz] - Tf[Nz - 1]) / dz ^ 2) + rhof_avg * uf_avg / epsilon * (hf[Nz - 1] - hf[Nz]) / dz - hv[Nz] * (Tf[Nz] - Ts[Nz]) / epsilon - U_wall * CN.pi * D_tank * (Tf[Nz] - T_amb) / (epsilon * A) - U_top * (Tf[Nz] - T_amb) / (epsilon * dz)) / rhof_avg;
    h_out := hf[Nz];
  end if;
...
//There is actually mass flowing
      Re[i] = rhof_avg * abs(uf_avg) * ds / muf[i];
//Use local superficial velocity
      Pr[i] = cpf[i] * muf[i] / kf[i];
      if Correlation == 1 then
        Nu[i] = 2 + 1.1 * Re[i] ^ 0.6 * Pr[i] ^ (1 / 3);
//Wakao and Kaguei
      elseif Correlation == 2 then
        Nu[i] = 2 + 0.47 * Re[i] ^ 0.5 * Pr[i] ^ 0.36;
//Use Melissari and Argyropolus
      elseif Correlation == 3 then
        Nu[i] = 2;
//Conservative
      elseif Correlation == 4 then
        Nu[i] = 2 + 0.664 * Re[i] ^ 0.5 * Pr[i] ^ (1 / 3);
//Only laminar
...
//Nie
    ...
    hv[i] = f_surface * 6 * (1 - epsilon) * Nu[i] * kf[i] / ds ^ 2;
